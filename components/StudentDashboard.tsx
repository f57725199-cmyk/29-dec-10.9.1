      ...
      if (activeTab === 'HOME') { 
          return (
              <div className="space-y-6 pb-24">
                  ...
                  {/* FEATURED SHORTCUTS (Admin Defined) */}
                  {settings?.featuredItems && settings.featuredItems.length > 0 && (
                      <div className="bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-2xl border border-indigo-100 shadow-sm">
                          <h3 className="font-bold text-indigo-900 mb-3 flex items-center gap-2">
                              <Zap size={18} className="text-yellow-500" /> 
                              {settings.featuredSectionTitle || 'Quick Access'}
                          </h3>
                          <div className="grid grid-cols-2 gap-3">
                              {settings.featuredItems.map(item => (
                                  <button 
                                      key={item.id}
                                      onClick={() => {
                                          onTabChange(item.type === 'MCQ' ? 'MCQ' : item.type === 'PDF' ? 'PDF' : 'VIDEO');
                                          // We need to trigger the selection flow
                                          // Since StudentDashboard handles 'selectedSubject' state internally when not passed from App...
                                          // Wait, App manages state. But StudentDashboard has local 'selectedSubject' for CONTENT tabs if drills down.
                                          // Actually `renderContentSection` uses `selectedSubject` and `selectedChapter` from StudentDashboard state.
                                          setSelectedSubject(item.subject);
                                          handleContentChapterSelect(item.chapter); // This sets step to PLAYER and FullScreen
                                      }}
                                      className="bg-white p-3 rounded-xl border border-indigo-100 shadow-sm hover:shadow-md transition-all text-left group"
                                  >
                                      <div className="flex justify-between items-start mb-2">
                                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${item.type === 'MCQ' ? 'bg-purple-100 text-purple-600' : item.type === 'PDF' ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>
                                              {item.type}
                                          </div>
